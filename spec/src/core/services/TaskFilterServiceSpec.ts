...
import {TaskFilterService} from '../../../../src/core/services/TaskFilterService';
import {TaskParser} from '../../../../src/core/services/TaskParser';
import {ArrayFn} from '../../../../src/core/utility/ArrayFunctions';
import { IoC } from '../../../../src/core/utility/IoC';
import { IConfig } from '../../../../src/core/domain/IConfig';

describe("TaskFilterService", function() {
    let config: IConfig = IoC.IConfig;
    let filterService: TaskFilterService = new TaskFilterService(config);
    ...
    describe("Filter()", () => {
        let config: IConfig = IoC.IConfig;
        let filter: TaskFilterService = new TaskFilterService(config);
        ...
        it('hides completed items if showCompletedItems = false', () => {
            let task1: Task = parser.Parse('x Download Todo.txt mobile app @Phone +coding @work  +todotxt');
            let task2: Task = parser.Parse('x a completed task starts with an x');
            let task3: Task = parser.Parse('x 2014-04-22 completion date appears directly after the x. @work');
            let task4: Task = parser.Parse('(C) Add cover sheets @Office +TPSReports');
            let task5: Task = parser.Parse('@play  Plan backyard herb garden @Home @computer +Garden    +House (more notes) @work');
            let task6: Task = parser.Parse('(A) x This one has a priority and is not completed @work');
            let task7: Task = parser.Parse('x 2013-06-12    2013-07-01  @studio lots of strange   space in this one +project2   @tommy  @the-cat  +project3 ');
            let task8: Task = parser.Parse('lower case test @phone');
            ...
            expect(filteredTasks[0].equals(task1)).toBeTruthy();
            expect(filteredTasks[1].equals(task3)).toBeTruthy();
            expect(filteredTasks[2].equals(task5)).toBeTruthy();
            expect(filteredTasks[3].equals(task6)).toBeTruthy();

            filteredTasks = filter.Filter(tasks, "@work", false);
            expect(filteredTasks.length).toEqual(2);
            expect(filteredTasks[0].equals(task5)).toBeTruthy();
            expect(filteredTasks[1].equals(task6)).toBeTruthy();
        });

        it('hides active items if showActiveItems = false', () => {
            let task1: Task = parser.Parse('x Download Todo.txt mobile app @Phone +coding @work  +todotxt');
            let task2: Task = parser.Parse('x a completed task starts with an x');
            let task3: Task = parser.Parse('x 2014-04-22 completion date appears directly after the x. @work');
            let task4: Task = parser.Parse('(C) Add cover sheets @Office +TPSReports');
            ...
            filteredTasks = filter.Filter(tasks, "@work", true, false);
            expect(filteredTasks.length).toEqual(2);
            expect(filteredTasks[0].equals(task1)).toBeTruthy();
            expect(filteredTasks[1].equals(task3)).toBeTruthy();
        });

        it('hides completed items if showCompletedItems = false and filter text is empty', () => {
            let task1: Task = parser.Parse('x Download Todo.txt mobile app @Phone +coding @work  +todotxt');
            let task2: Task = parser.Parse('x a completed task starts with an x');
            let task3: Task = parser.Parse('x 2014-04-22 completion date appears directly after the x. @work');
            let task4: Task = parser.Parse('(C) Add cover sheets @Office +TPSReports');
            ...
            let filteredTasks = filter.Filter(tasks, "");
            expect(filteredTasks.length).toEqual(8);
            expect(filteredTasks[0].equals(task1)).toBeTruthy();
            expect(filteredTasks[1].equals(task2)).toBeTruthy();
            expect(filteredTasks[2].equals(task3)).toBeTruthy();
            expect(filteredTasks[3].equals(task4)).toBeTruthy();
            expect(filteredTasks[4].equals(task5)).toBeTruthy();
            expect(filteredTasks[5].equals(task6)).toBeTruthy();
            expect(filteredTasks[6].equals(task7)).toBeTruthy();
            expect(filteredTasks[7].equals(task8)).toBeTruthy();

            filteredTasks = filter.Filter(tasks, "", false);
            expect(filteredTasks.length).toEqual(4);
            expect(filteredTasks[0].equals(task4)).toBeTruthy();
            expect(filteredTasks[1].equals(task5)).toBeTruthy();
            expect(filteredTasks[2].equals(task6)).toBeTruthy();
            expect(filteredTasks[3].equals(task8)).toBeTruthy();
        });

        it('hides active items if showActiveItems = false and filter text is empty', () => {
            let task1: Task = parser.Parse('x Download Todo.txt mobile app @Phone +coding @work  +todotxt');
            let task2: Task = parser.Parse('x a completed task starts with an x');
            let task3: Task = parser.Parse('x 2014-04-22 completion date appears directly after the x. @work');
            let task4: Task = parser.Parse('(C) Add cover sheets @Office +TPSReports');
            ...
            filteredTasks = filter.Filter(tasks, "", true, false);
            expect(filteredTasks.length).toEqual(4);
            expect(filteredTasks[0].equals(task1)).toBeTruthy();
            expect(filteredTasks[1].equals(task2)).toBeTruthy();
            expect(filteredTasks[2].equals(task3)).toBeTruthy();
    ...
    describe("SaveFilter()", () => {
        it('saves the given filter', () => {
            let savedFiltersBefore: Array<string> = filterService.LoadSavedFilters();
            let filter: string = Math.floor(Math.random() * Math.floor(10000000)).toString();
            filterService.SaveFilter(filter);
            let savedFiltersAfter: Array<string> = filterService.LoadSavedFilters();

            expect(savedFiltersAfter.length).toEqual(savedFiltersBefore.length + 1);
            expect(ArrayFn.IndexWhere(savedFiltersBefore, (f) => {return f === filter})).toEqual(-1);
            expect(ArrayFn.IndexWhere(savedFiltersAfter, (f) => {return f === filter}) > -1).toBeTruthy();
    ...
    describe("DeleteFilter()", () => {
        it('deletes the given filter', () => {
            ...
            expect(savedFiltersAfter.length).toEqual(savedFiltersBefore.length - 1);
            expect(ArrayFn.IndexWhere(savedFiltersBefore, (f) => {return f === filter}) > -1).toBeTruthy();
            expect(ArrayFn.IndexWhere(savedFiltersAfter, (f) => {return f === filter})).toEqual(-1);
        });
    });

    describe("LoadSavedFilters()", () => {
        it('loads the saved filters', () => {
            let filter: string = Math.floor(Math.random() * Math.floor(10000000)).toString();
            filterService.SaveFilter(filter);
            let savedFilters: Array<string> = filterService.LoadSavedFilters();

            expect(savedFilters.length > 0).toBeTruthy();
            expect(ArrayFn.IndexWhere(savedFilters, (f) => {return f === filter}) > -1).toBeTruthy();
            filterService.DeleteFilter(filter);
        });
    });
